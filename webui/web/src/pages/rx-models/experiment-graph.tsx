...
class ExperimentGraph extends GraphCore<BaseNode, BaseEdge> {
  ...
  // 当前选中的群组
  selectedGroup$: BehaviorSubject<X6GroupNode | undefined> =
    new BehaviorSubject<X6GroupNode | undefined>(undefined);
    ...
    super({
      ...
      connecting: {
        ...
        validateConnection({
          ...
          const node = targetView!.cell as X6Node;
  ...
  renderEdge(edgeMeta: EdgeMeta): BaseEdge | undefined {
    const { type } = edgeMeta;
    if (type === "group") {
      return this.graph!.addEdge(new X6GroupEdge(edgeMeta)) as BaseEdge;
    }
  ...
  onSelectNodes(nodes: BaseNode[]) {
    const selectedNodes: X6Node[] = nodes.filter(
      (cell) => cell.isNode() && !cell.isGroup()
    ) as X6Node[];
    const selectedGroups: X6GroupNode[] = nodes.filter(
      (cell) => cell.isNode() && cell.isGroup()
    );
  ...
  async onConnectNode(args: any) {
    ...
    console.warn(
      "onConnectNode: " + JSON.stringify(edge) + "|" + JSON.stringify(isNew)
    );
  ...
  async onConnectionRemoved(args: ConnectionRemovedArgs) {
    try {
      const { edge } = args;
  ...
  onDeleteNodeOrEdge(args: { nodes: BaseNode[]; edges: GuideEdge[] }) {
    ...
    const normalNodes: X6Node[] = nodes.filter(
      (node) => !node.isGroup()
    ) as X6Node[];
